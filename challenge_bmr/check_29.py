...
# ─── Inlined config ──────────────────────────────────────────────────────────
CHECK_29_CONFIG = {
    "name": "Actual_issued_quantity_does_not_match_standard_quantity.",
    "process": "Material Requisition Verification",
}

def get_check_name(check_id: str) -> str:
    return CHECK_29_CONFIG["name"]

def get_check_process(check_id: str) -> str:
    return CHECK_29_CONFIG["process"]
...
if __name__ == "__main__":
    ...
    # Handle MongoDB document format: extract page list from steps.extract_page_data
    if isinstance(raw, dict) and "steps" in raw:
        bmr_data = raw["steps"].get("extract_page_data", [])
    elif isinstance(raw, list):
        bmr_data = raw
    else:
        bmr_data = raw

    print(f"Loaded {len(bmr_data)} BMR pages")

    with open(attachment_filepath, "r", encoding="utf-8") as f:
        attachment_data = json.load(f)

    print(f"Loaded {len(attachment_data)} attachment pages")
