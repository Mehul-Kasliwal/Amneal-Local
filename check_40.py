...
class Actual_BMR_Reconciliation_Protocol:
    """
    Check 40: Reconciliation Protocol Check
    ---
    Description:
    ...
    3. LLM extracts symbol-value mappings and formula expressions for ALL computed stages.
    4. Python computes every expression using ast-based safe evaluation (deterministic).
    5. Compares Python-computed values against reported values for each stage.
    6. Flag anomaly if any computed stage has a mismatch beyond tolerance.
    ---
    ...
    CHECK_NAME = "Reconciliation Protocol Check"
...
"""
    ...
    def _verify_all_stages(self, llm_output: Dict) -> Tuple[int, List[Dict]]:
        """
        Verify all stages that have expressions using Python computation.

        Returns:
            (anomaly_status, verification_details) tuple
            anomaly_status: 0 if all match, 1 if any mismatch
            verification_details: list of per-stage verification results
        """
        ...
        for stage in stages:
            ...
            if not expression:
                detail["match"] = True  # No formula to verify
            ...
            if computed is None:
                ...
                detail["match"] = True  # Can't verify, don't flag
                verification_details.append(detail)
                continue
            ...
            try:
                ...
                detail["match"] = True  # Can't compare, don't flag
    ...
    def run(self, document_pages: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        """
        Run Reconciliation Protocol checks across all pages.
        Returns:
        ...
        for idx, page in enumerate(document_pages, 1):
            ...
            try:
                ...
                # Verify all stages using Python computation
                anomaly, verification_details = self._verify_all_stages(llm_output)
                ...
                # Find mismatched stages for anomaly detail
                ...
                if mismatches:
                    ...
                    debug_entry["skip_reason"] = "All stages match (within tolerance) or no expressions found"
...
if __name__ == "__main__":
    ...
    # complete_data_path = "/home/softsensor/Desktop/Amneal/Amneal-Local/challenge_bmr/05jan_AH250076_50Checks 1.json"
            ...
            if "llm_output" in d:
                ...
                for v in d.get("verification_details", []):
                    if v.get("expression"):
                        ...
                        print(f"  {status} {v['name']} ({v['symbol']}): "
